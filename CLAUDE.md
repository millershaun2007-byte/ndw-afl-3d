...
- **Resolved 2026-08-11, and the framing above was wrong in a specific way worth naming**: there is no Mixamo anywhere in this repo — confirmed by grepping the whole tree, three hits, all prose, no FBX, no Mixamo asset ever existed here. "Mixamo or otherwise" in this file's own earlier wording was itself the source of the confusion: it wrote a constraint against a tool nobody used, and Day1RuckContest.cs's header comment then cited that constraint to justify destroying every Animator on Day 1's characters. The actual visual test this section called for — drop a real clip on it, watch the limbs — has now been run, via a direct render (`AvatarRetargetTest.cs`), and the answer has two parts. A Humanoid-avatar-based retarget of CrocWalking.glb onto Croc produced a genuinely broken pose (arms clenched near the jaw, confirmed by screenshot, independent of the clip — even a zero-muscle "T-pose" render came out wrong). But Generic playback (bone-name matching, no avatar, no muscle space) of the exact same clip produced a natural, correct mid-stride walk on both Croc and Roo — this is not a new capability, it's exactly what BuildScript.cs's six-player game already ships with. Humanoid retargeting was never actually needed by anything in this project (no external clip exists to retarget onto anything), so the Humanoid-avatar defect is real but not load-bearing — don't chase it further unless an actual external clip shows up. Day 1 now assigns the existing `Assets/_Generated/CrocRiggedAIAnimator.controller` / `RooRiggedAIAnimator.controller` (Generic, Idle state) instead of destroying the Animator.
